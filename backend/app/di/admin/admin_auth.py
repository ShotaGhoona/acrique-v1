...
from app.application.use_cases.admin.admin_auth_usecase import AdminAuthUsecase
...
def get_admin_auth_usecase(session: Session = Depends(get_db)) -> AdminAuthUsecase:
    """AdminAuthUsecaseを取得（依存性注入）

    Args:
        session: DBセッション

    Returns:
        AdminAuthUsecaseインスタンス
    """
