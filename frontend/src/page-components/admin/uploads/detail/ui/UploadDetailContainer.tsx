...
import { ArrowLeft, Download, Check, X, RotateCcw, FileText, User, Package } from 'lucide-react';
...
import { getUploadById, uploadStatusLabels, uploadStatusColors } from '../../dummy-data/uploads';

interface UploadDetailContainerProps {
  uploadId: string;
}

export function UploadDetailContainer({ uploadId }: UploadDetailContainerProps) {
          ...
          <Card>
            ...
            <CardContent>
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <span className="text-sm text-muted-foreground">ファイル名</span>
                ...
                <div>
                  <span className="text-sm text-muted-foreground">ファイル形式</span>
                  <p className="font-medium">{upload.fileType}</p>
                </div>
                <div>
                  <span className="text-sm text-muted-foreground">アップロード日時</span>
                  <p className="font-medium">{upload.uploadedAt}</p>
                </div>
              </div>

              <div className="mt-4">
                <Button onClick={() => alert(`ダウンロード: ${upload.fileName}（未実装）`)}>
            ...
            <Card>
              ...
              <CardContent>
                <div className="rounded-md bg-muted p-3 text-sm">
                  ...
                  {upload.reviewedAt && (
                    <p className="mt-2 text-xs text-muted-foreground">{upload.reviewedAt}</p>
                  )}
                </div>
              </CardContent>
            </Card>
