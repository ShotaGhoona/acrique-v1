...
export interface ApiResourceProps {
  ...
  /**
   * ECRリポジトリ（バックエンドイメージ用）
   * 指定するとECRからイメージを取得（タグ: latest）
   */
...
export class ApiResource extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ApiResourceProps) {
    ...
    // コンテナイメージの決定
    // 優先順位: 1. containerImage（明示指定） 2. ECRリポジトリ 3. nginx:latest（プレースホルダー）
    let containerImage: ecs.ContainerImage;
    if (props.containerImage) {
      containerImage = props.containerImage;
    } else if (props.ecrRepository) {
      // ECRリポジトリから latest タグのイメージを使用
      // CI/CDでイメージをプッシュ後、ECSサービスを更新することで新しいイメージが使われる
      containerImage = ecs.ContainerImage.fromEcrRepository(props.ecrRepository, 'latest');
    } else {
      // プレースホルダー（初回デプロイ用）
